...
# -------------------------------------------------------------
# SMRITI RAWAT PORTFOLIO PATTERN: PHYSICAL JOURNAL BOOK
# - OUTSIDE STICKING-OUT LEATHER INDEX TABS ON RIGHT MARGIN
# - EXACT HARDCOVER BACKING & PROPORTIONAL SIZING
# - 100% BRAND LOGOS & EXACT PAGES 01-04 SPECIFICATION
# -------------------------------------------------------------
SMRITI_JOURNAL_HTML = """<!-- ABOUT ME: PHYSICAL SMRITI RAWAT STYLE JOURNAL BOOK -->
<section class="relative bg-[#f4efe6] text-[#1F1F1F] py-24 overflow-hidden border-b border-amber-900/10 z-20" id="about-journal">
  <div class="max-w-7xl mx-auto px-4 md:px-8 text-center">
    ...
    <div class="relative w-full max-w-[980px] mx-auto min-h-[600px] flex items-center justify-center">
      ...
      <div id="smriti-book-cover" class="cursor-pointer transition-all duration-500 z-40 group" onclick="openSmritiBook()">
        <div class="relative">
          <!-- Cover Book Image (STABLE, EXACT PROPORTION) -->
          <div class="w-[330px] md:w-[430px] h-[560px] rounded-[30px] shadow-[0_30px_70px_-15px_rgba(0,0,0,0.35)] overflow-hidden relative bg-[#36132b] border border-amber-900/30">
      ...
      <!-- ==================== OPENED PHYSICAL BOOK ==================== -->
      <div id="smriti-book-opened" class="hidden w-full transition-opacity duration-500 opacity-0 z-30">
        
        <!-- HARDCOVER BACKBOARD CONTAINER WITH OUTSIDE RIGHT TABS -->
        <div class="relative w-full bg-[#36132b] rounded-[36px] p-2.5 md:p-3.5 shadow-[0_35px_80px_-20px_rgba(0,0,0,0.35)]">
          
          <!-- OUTSIDE RIGHT VERTICAL INDEX TABS (STICKING OUT BEYOND THE BOOK COVER) -->
          <div class="absolute top-10 -right-10 md:-right-12 flex flex-col gap-2 z-50">
            
            <!-- Home / Cover Tab -->
            <button onclick="closeSmritiBook()" class="w-10 md:w-12 h-11 bg-[#ebdcc4] hover:bg-[#8b2252] text-gray-800 hover:text-white rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/20 flex items-center justify-center transition-all hover:translate-x-1" title="Close to Cover">
            ...
            <!-- Tab 3: Lessons -->
            <button onclick="switchSmritiSpread('spread2')" class="px-2 md:px-2.5 py-3.5 bg-[#c2a297] text-white text-[10px] font-mono font-bold rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1 hidden sm:flex" style="writing-mode: vertical-rl; text-orientation: mixed;">
              Lessons
            </button>
          ...
          <!-- INNER PAGES SPREAD 1 (PAGES 01 & 02) -->
          <div id="smriti-spread-1" class="w-full bg-[#faf6ee] rounded-[28px] p-6 md:p-10 relative overflow-hidden text-left min-h-[550px]" style="background-image: radial-gradient(#d6cebe 1px, transparent 1px); background-size: 20px 20px;">
            
            <!-- TOP-RIGHT CLOSE BUTTON '✕' (MATCHING SMRITI RAWAT SITE) -->
            <button onclick="closeSmritiBook()" class="absolute top-4 right-5 text-gray-400 hover:text-[#8b2252] text-xl font-bold z-40 p-1 transition-colors" title="Close Journal">
              ✕
            </button>
          ...
          <div id="smriti-spread-2" class="hidden w-full bg-[#faf6ee] rounded-[28px] p-6 md:p-10 relative overflow-hidden text-left min-h-[550px]" style="background-image: radial-gradient(#d6cebe 1px, transparent 1px); background-size: 20px 20px;">
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + SMRITI_JOURNAL_HTML + '\n\n' + content[end_pos:]
    print("Successfully replaced with Smriti Rawat style physical journal book & outside tabs!")
