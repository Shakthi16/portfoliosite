...
# -------------------------------------------------------------
# GOOGLE FONTS IMPORT (INSTRUMENT SERIF, CORMORANT GARAMOND, COURIER PRIME, IBM PLEX MONO, CAVEAT)
# -------------------------------------------------------------
FONTS_PRELOAD = """<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Caveat:wght@500;700&family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,600&family=Courier+Prime:ital,wght@0,400;0,700;1,400&family=IBM+Plex+Mono:wght@400;500;600&family=Instrument+Serif:ital@0;1&family=Special+Elite&display=swap" rel="stylesheet">"""

# Replace existing font link or head
if 'Instrument+Serif' not in content:
    content = content.replace('</head>', FONTS_PRELOAD + '\n</head>')

# -------------------------------------------------------------
# EDITORIAL JAPANESE MUJI STATIONERY / KINFOLK JOURNAL SPECIFICATION
# 1. Main Headings: Instrument Serif / Cormorant Garamond
# 2. Body Text: Courier Prime / Special Elite
# 3. Small Labels: IBM Plex Mono
# 4. Handwritten Annotations: Caveat
# 5. Right Side Tabs: #7A2D4F to #5B1F39 (18px radius, soft shadow)
# -------------------------------------------------------------
MUJI_EDITORIAL_JOURNAL_HTML = """<!-- REALISTIC DECKLE-EDGE TORN PAPER SEPARATION DIVIDER (TOP) -->
...
<!-- ABOUT ME: EDITORIAL MUJI STYLE PHYSICAL JOURNAL BOOK -->
<section class="relative bg-[#FAF7F2] text-[#1F1F1F] py-8 md:py-12 overflow-hidden border-b border-amber-900/10 z-20" id="about-journal">
  <div class="max-w-7xl mx-auto px-4 md:px-8 text-center">
    ...
    <!-- Title Header -->
    <div class="mb-6">
      <span class="text-[11px] uppercase tracking-[0.3em] text-[#7B3152] font-mono font-bold block mb-1" style="font-family: 'IBM Plex Mono', monospace;">ABOUT ME</span>
      <h2 class="text-3xl md:text-5xl font-bold italic text-[#1F1F1F]" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">The Personal Journal</h2>
    </div>
    ...
    <div class="relative w-full max-w-[1000px] mx-auto min-h-[530px] md:min-h-[560px] flex items-center justify-center">
      ...
      <div id="smriti-book-opened" class="hidden w-full transition-opacity duration-500 opacity-0 z-30">
        ...
        <!-- SLEEK BOOK CONTAINER: BORDER NO! (0px PADDING, SLEEK 1px OUTER STROKE) -->
        <div class="relative w-full rounded-[22px] shadow-[0_20px_50px_rgba(0,0,0,0.18)] border border-[#5A213B]/70 bg-[#36132b] p-[1.5px]">
          
          <!-- OUTSIDE RIGHT VERTICAL INDEX TABS (GRADIENT #7A2D4F -> #5B1F39, RADIUS 18px) -->
          <div class="absolute top-6 -right-9 md:-right-11 flex flex-col gap-2 z-50">
            
            <!-- Home / Cover Tab -->
            <button onclick="closeSmritiBook()" class="w-9 md:w-11 h-10 bg-[#EBDCC4] hover:bg-[#7B3152] text-gray-800 hover:text-white rounded-r-[18px] shadow-[0_6px_20px_rgba(0,0,0,0.08)] border-t border-r border-b border-amber-900/20 flex items-center justify-center transition-all hover:translate-x-1" title="Close to Cover">
            ...
            <button id="tab-btn-spread1" onclick="switchSmritiSpread('spread1')" class="px-2 md:px-2.5 py-4 bg-gradient-to-b from-[#7A2D4F] to-[#5B1F39] text-white text-[11px] font-mono font-bold rounded-r-[18px] shadow-[0_6px_20px_rgba(0,0,0,0.08)] border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1" style="writing-mode: vertical-rl; text-orientation: mixed; font-family: 'IBM Plex Mono', monospace;">
              About Me
            </button>

            <!-- Tab 2: Things I Learnt (P. 03-04) -->
            <button id="tab-btn-spread2" onclick="switchSmritiSpread('spread2')" class="px-2 md:px-2.5 py-4 bg-[#8A4765] hover:bg-[#7A2D4F] text-white text-[11px] font-mono font-bold rounded-r-[18px] shadow-[0_6px_20px_rgba(0,0,0,0.08)] border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1" style="writing-mode: vertical-rl; text-orientation: mixed; font-family: 'IBM Plex Mono', monospace;">
              Things I Learnt
            </button>

            <!-- Tab 3: Story Writing -->
            <button id="tab-btn-spread3" onclick="switchSmritiSpread('spread3')" class="px-2 md:px-2.5 py-4 bg-[#C2A297] hover:bg-[#7A2D4F] text-white text-[11px] font-mono font-bold rounded-r-[18px] shadow-[0_6px_20px_rgba(0,0,0,0.08)] border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1 hidden sm:flex" style="writing-mode: vertical-rl; text-orientation: mixed; font-family: 'IBM Plex Mono', monospace;">
              Story Writing
            </button>

          </div>

          <!-- INNER PAGES SPREAD 1 (PAGES 01 & 02 - EXACT TYPOGRAPHY & MUJI GRID) -->
          <div id="smriti-spread-1" class="w-full bg-[#FAF7F2] rounded-[20px] p-6 md:p-8 relative overflow-hidden text-left h-[510px] md:h-[535px] flex flex-col justify-between" style="background-image: radial-gradient(#D9CFC3 1.2px, transparent 1.2px); background-size: 24px 24px;">
            
            <!-- TOP-RIGHT CLOSE BUTTON '✕' -->
            <button onclick="closeSmritiBook()" class="absolute top-4 right-5 text-gray-500 hover:text-[#7B3152] text-xl font-bold z-40 p-1 transition-colors" title="Close Journal">
              ✕
            </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 relative z-10 h-full overflow-y-auto md:overflow-hidden">
              ...
              <div class="flex flex-col justify-between text-left pr-0 md:pr-2">
                <div>
                  <h3 class="text-4xl md:text-5xl font-bold text-[#1F1F1F] mb-1 tracking-tight" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Shakthi Sri</h3>
                  
                  <!-- Pill Badges -->
                  <div class="flex flex-wrap items-center gap-2 mb-3">
                    <span class="px-3 py-0.5 bg-[#F0E3DB] text-[#7B3152] text-[10px] font-mono font-bold rounded-full border border-[#D8C2B5] tracking-wider uppercase" style="font-family: 'IBM Plex Mono', monospace;">DEVELOPER &amp; RESEARCHER</span>
                    <span class="text-base text-gray-700 underline decoration-amber-900/30" style="font-family: 'Caveat', cursive;">class of 2026</span>
                  </div>

                  <!-- Timeline List with Icons -->
                  <div class="space-y-2 font-mono text-[11px] text-gray-800 font-medium mb-3" style="font-family: 'IBM Plex Mono', monospace;">
                    ...
                    <div class="flex justify-between items-center border-b border-dashed border-amber-900/15 pb-1">
                      ...
                      <span class="text-[#7B3152] font-bold">CGPA: 8.6</span>
                    </div>
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-2">
                    ...
                    <div class="bg-[#F7F0E6] p-3 rounded-2xl border border-amber-900/15 shadow-sm relative">
                      ...
                      <span class="text-base text-[#7B3152] font-bold block mb-1" style="font-family: 'Caveat', cursive;">about me ヾ</span>
                      <ul class="text-[11px] text-gray-800 leading-snug space-y-1" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                    ...
                    <div class="bg-[#F7F0E6] p-3 rounded-2xl border border-amber-900/15 shadow-sm relative">
                      <div class="flex items-center justify-between mb-1">
                        ...
                        <span class="text-xs text-amber-900">☆</span>
                      </div>
                      <span class="text-[9px] font-mono font-bold text-gray-500 uppercase block mb-0.5" style="font-family: 'IBM Plex Mono', monospace;">TODAY</span>
                      <p class="text-[11px] text-gray-800 leading-relaxed" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                ...
                <div class="pt-2 border-t border-dashed border-amber-900/20 flex justify-between items-center gap-2">
                  <div class="bg-[#F0E8DC] px-2.5 py-1.5 rounded-xl text-[10px] font-mono text-gray-800 border border-amber-900/15" style="font-family: 'IBM Plex Mono', monospace;">
                  ...
                  <div class="text-right">
                    <p class="text-base text-[#5A213B] font-bold leading-tight" style="font-family: 'Caveat', cursive;">“ code with purpose,<br/>create with impact. ♡</p>
                    <div class="w-12 h-0.5 bg-[#5A213B]/40 ml-auto mt-0.5"></div>
                  </div>
                </div>
              ...
              <div class="flex flex-col justify-between relative text-left pl-0 md:pl-2">
                ...
                <div class="flex justify-between items-start mb-2">
                  <div class="text-center">
                    ...
                    <span class="block text-[9px] text-gray-600" style="font-family: 'Caveat', cursive;">breathe.png</span>
                  </div>
                  <div class="flex items-center gap-4" style="font-family: 'IBM Plex Mono', monospace;">
                    <div class="text-center">
                      <div class="w-7 h-7 bg-[#7B3152]/10 rounded-lg flex items-center justify-center text-[#7B3152] text-xs font-bold mx-auto">📁</div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center mb-3">
                  ...
                  <div class="sm:col-span-6 border border-dashed border-amber-900/30 rounded-2xl p-3 bg-[#FBF7F0]">
                    <span class="text-base text-[#7B3152] font-bold block mb-1" style="font-family: 'Caveat', cursive;">currently ヾ</span>
                    <ul class="text-[11px] text-gray-800 leading-snug space-y-0.5" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                  ...
                  <div class="sm:col-span-6 flex justify-center relative">
                    <div class="relative">
                      ...
                      <div class="absolute -top-2.5 right-0 bg-black/80 text-white font-mono text-[8px] font-bold px-2.5 py-0.5 rounded-full shadow-md z-30" style="font-family: 'IBM Plex Mono', monospace;">
                        @Shakthi.16
                      </div>
                    </div>
                  </div>
                ...
                <div class="border border-dashed border-amber-900/30 rounded-2xl p-2.5 bg-[#FBF7F0] mb-2">
                  <span class="text-base text-[#7B3152] font-bold block mb-1.5" style="font-family: 'Caveat', cursive;">tech i work with ヾ</span>
                  <div class="flex flex-wrap items-center justify-between gap-2 text-center" style="font-family: 'IBM Plex Mono', monospace;">
                    <div class="text-center">
                ...
                <div class="border border-dashed border-amber-900/30 rounded-2xl p-2 bg-[#FBF7F0] relative">
                  <span class="text-base text-[#7B3152] font-bold block mb-1 text-center" style="font-family: 'Caveat', cursive;">what i build ♡</span>
                  <div class="grid grid-cols-3 gap-1.5 text-center text-[9px] font-mono text-gray-700 font-bold" style="font-family: 'IBM Plex Mono', monospace;">
          ...
          <!-- INNER PAGES SPREAD 2 (PAGES 03 & 04 - EXACT TYPOGRAPHY & MUJI GRID) -->
          <div id="smriti-spread-2" class="hidden w-full bg-[#FAF7F2] rounded-[20px] p-6 md:p-8 relative overflow-hidden text-left h-[510px] md:h-[535px] flex flex-col justify-between" style="background-image: radial-gradient(#D9CFC3 1.2px, transparent 1.2px); background-size: 24px 24px;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 relative z-10 h-full overflow-y-auto md:overflow-hidden">
              ...
              <!-- PAGE 03: ON BUILDING. JOURNAL ENTRY 03 -->
              <div class="space-y-2 text-left pr-0 md:pr-2 flex flex-col justify-between">
                <div>
                  <div class="flex items-center gap-2 mb-1">
                    <span class="px-1.5 py-0.5 bg-[#F0E3DB] text-[#7B3152] font-mono font-bold text-[9px] rounded border border-amber-900/20" style="font-family: 'IBM Plex Mono', monospace;">03</span>
                    <div>
                      <h3 class="text-3xl md:text-4xl font-bold text-[#1F1F1F] leading-tight" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">On Building. ヾ</h3>
                      <p class="text-[10px] font-mono text-gray-500" style="font-family: 'IBM Plex Mono', monospace;">Journal Entry — 03</p>
                    </div>
                  </div>

                  <p class="text-[11px] text-gray-800 leading-relaxed mb-2" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                  ...
                  <ul class="text-[11px] text-gray-800 space-y-0.5 mb-2" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                  ...
                  <p class="text-[11px] text-gray-800 leading-relaxed mb-2" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                    ...
                    <span class="underline decoration-amber-900/40 font-bold text-[#7B3152]">Good engineering begins with understanding.</span> Everything else follows.
                  </p>
                </div>

                <!-- Middle Taped Quote Card & Bridge Sketch -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 items-center my-1">
                  <div class="bg-[#F7F0E6] p-2.5 rounded-xl border border-amber-900/15 text-[11px] text-gray-800 relative" style="font-family: 'Caveat', cursive;">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-dashed border-amber-900/20">
                  <div class="bg-[#FBF7F0] p-2 rounded-xl border border-amber-900/15 text-[10.5px] text-gray-800" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                    <p class="font-bold text-[#7B3152] mb-0.5" style="font-family: 'IBM Plex Mono', monospace;">A small note I often return to:</p>
                  ...
                  <div class="text-left text-[11px] text-gray-800 flex flex-col justify-between" style="font-family: 'Caveat', cursive;">
                    <span class="font-bold text-[#5A213B] block mb-0.5 text-xs font-mono" style="font-family: 'IBM Plex Mono', monospace;">Reflection</span>
              ...
              <div class="space-y-2 text-left pl-0 md:pl-2 flex flex-col justify-between">
                <div>
                  <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center gap-2">
                      <span class="px-1.5 py-0.5 bg-[#F0E3DB] text-[#7B3152] font-mono font-bold text-[9px] rounded border border-amber-900/20" style="font-family: 'IBM Plex Mono', monospace;">04</span>
                      <h3 class="text-3xl md:text-4xl font-bold text-[#1F1F1F]" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Things I've Learned. ♡</h3>
                    </div>
                  </div>

                  <!-- 4 Circle Icon Items -->
                  <div class="space-y-1.5 text-[11px] text-gray-800 mb-3" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                    <div class="flex items-start gap-2">
                      <div class="w-6 h-6 rounded-full bg-amber-900/10 flex items-center justify-center text-[10px] font-mono font-bold text-[#7B3152] shrink-0 mt-0.5" style="font-family: 'IBM Plex Mono', monospace;">&lt;/&gt;</div>
                      <div>
                        <h4 class="font-bold text-[#5A213B] text-sm" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">The best code usually disappears.</h4>
                    ...
                    <div class="flex items-start gap-2">
                      <div class="w-6 h-6 rounded-full bg-amber-900/10 flex items-center justify-center text-[10px] font-mono font-bold text-[#7B3152] shrink-0 mt-0.5" style="font-family: 'IBM Plex Mono', monospace;">🕒</div>
                      <div>
                        <h4 class="font-bold text-[#5A213B] text-sm" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Speed is temporary.</h4>
                    ...
                    <div class="flex items-start gap-2">
                      <div class="w-6 h-6 rounded-full bg-amber-900/10 flex items-center justify-center text-[10px] font-mono font-bold text-[#7B3152] shrink-0 mt-0.5" style="font-family: 'IBM Plex Mono', monospace;">📄</div>
                      <div>
                        <h4 class="font-bold text-[#5A213B] text-sm" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Documentation is an act of respect.</h4>
                    ...
                    <div class="flex items-start gap-2">
                      <div class="w-6 h-6 rounded-full bg-amber-900/10 flex items-center justify-center text-[10px] font-mono font-bold text-[#7B3152] shrink-0 mt-0.5" style="font-family: 'IBM Plex Mono', monospace;">🌱</div>
                      <div>
                        <h4 class="font-bold text-[#5A213B] text-sm" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Learning is never finished.</h4>
                  ...
                  <div class="p-2 bg-[#F7F0E6] rounded-xl border border-amber-900/15 text-xs text-gray-800 text-center mb-2 relative" style="font-family: 'Caveat', cursive;">
                    <div class="absolute -top-2 left-1/2 -translate-x-1/2 w-6 h-2 bg-amber-200/80 rotate-[1deg]"></div>
                ...
                <div class="pt-2 border-t border-dashed border-amber-900/20">
                  <div class="flex items-center justify-between mb-1">
                    <h4 class="font-bold text-base text-[#5A213B]" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Working With Others. 👥</h4>
                  </div>

                  <p class="text-[11px] text-gray-700 leading-tight mb-1.5" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
                  ...
                  <div class="bg-[#F7F0E6] p-1.5 rounded-lg border border-amber-900/15 text-xs text-gray-800 text-center mb-1.5" style="font-family: 'Caveat', cursive;">
                    "The most valuable work is often the work no one notices because everything simply functions as expected. ♡"
                  </div>

                  <ul class="text-[10.5px] text-gray-800 space-y-0.5" style="font-family: 'Courier Prime', 'Special Elite', monospace;">
          ...
          <div id="smriti-spread-3" class="hidden w-full bg-[#FAF7F2] rounded-[20px] p-6 md:p-8 relative overflow-hidden text-left h-[510px] md:h-[535px] flex flex-col justify-between" style="background-image: radial-gradient(#D9CFC3 1.2px, transparent 1.2px); background-size: 24px 24px;">
            ...
            <div class="flex flex-col items-center justify-center h-full text-center p-8">
              ...
              <h3 class="text-3xl font-bold text-[#1F1F1F] mb-2" style="font-family: 'Instrument Serif', 'Cormorant Garamond', serif;">Story Writing</h3>
...
<script>
  ...
  function switchSmritiSpread(targetId) {
    ...
    Object.keys(spreads).forEach(key => {
      ...
      if (btns[key]) {
        if (key === targetId) {
          btns[key].classList.remove('bg-[#8A4765]', 'bg-[#C2A297]');
          btns[key].classList.add('bg-gradient-to-b', 'from-[#7A2D4F]', 'to-[#5B1F39]');
        } else {
          btns[key].classList.remove('bg-gradient-to-b', 'from-[#7A2D4F]', 'to-[#5B1F39]');
          btns[key].classList.add('bg-[#8A4765]');
        }
      }
    });
  }
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + MUJI_EDITORIAL_JOURNAL_HTML + '\n\n' + content[end_pos:]
    print("Successfully implemented exact Editorial Japanese Muji / Traveler's Notebook font system!")
