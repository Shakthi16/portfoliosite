...
# -------------------------------------------------------------
# OPENED PERSONAL JOURNAL SPREAD (PRIMARY & DIRECTLY VISIBLE)
# NO COVER BUTTON OVERLAYS! NO CLICK TO OPEN OVERLAYS!
# -------------------------------------------------------------
OPENED_JOURNAL_SPREAD_HTML = """<!-- ABOUT ME: OPENED PERSONAL JOURNAL SPREAD (DIRECTLY VISIBLE, ZERO OVERLAYS) -->
<section class="relative bg-[#f4efe6] text-[#1F1F1F] py-24 overflow-hidden border-b border-amber-900/10 z-20" id="about-journal">
  <div class="max-w-6xl mx-auto px-4 md:px-8 text-center">
    ...
    <!-- OPENED JOURNAL BOOK SPREAD (2-PAGE NOTEBOOK - MATCHING YOUR SPREAD) -->
    <div class="relative w-full max-w-[960px] mx-auto min-h-[580px] flex items-center justify-center">
      ...
      <div class="w-full bg-[#faf7f2] rounded-[32px] shadow-[0_30px_80px_-20px_rgba(0,0,0,0.18)] border border-amber-900/15 p-6 md:p-10 relative overflow-hidden text-left" style="background-image: radial-gradient(#d6cebe 1px, transparent 1px); background-size: 20px 20px;">
        ...
        <!-- Center Book Spine & Ribbon Bookmark -->
...
# Completely replace #about-journal section in index.html
pattern = re.compile(r'<!-- ABOUT ME:.*?</section>\s*(?:<script>.*?</script>)?', re.DOTALL)
if pattern.search(content):
    content = pattern.sub(OPENED_JOURNAL_SPREAD_HTML + '\n\n', content, count=1)
    print("Successfully replaced #about-journal section with OPENED SPREAD!")
else:
    print("Could not find #about-journal section to replace.")
