...
# -------------------------------------------------------------
# EXACT MATCH TO SMRITI RAWAT SPECIFICATION:
# 1. PAPER LAYER TRANSITION WAVE BETWEEN HOME & JOURNAL (IMAGE 1)
# 2. SLEEK THIN 2px BOOK BORDER MATCHING IMAGE 2 EXACTLY (NO CHUNKY BORDER)
# 3. OUTSIDE LEATHER TABS & 100% BRAND LOGOS
# -------------------------------------------------------------
EXACT_SMRITI_SPEC_HTML = """<!-- PAPER WAVE LAYER TRANSITION BETWEEN MAIN HOME PAGE & JOURNAL (MATCHING IMAGE 1) -->
<div class="w-full bg-[#f4efe6] relative z-20 pointer-events-none -mb-1 pt-6">
  <svg class="w-full h-8 md:h-12 text-[#f4efe6] fill-current" viewBox="0 0 1200 120" preserveAspectRatio="none">
    <path d="M0,40 C300,90 900,0 1200,40 L1200,120 L0,120 Z"></path>
  </svg>
</div>
  ...
  <div class="max-w-7xl mx-auto px-4 md:px-8 text-center">
    ...
    <div class="relative w-full max-w-[1000px] mx-auto min-h-[530px] md:min-h-[560px] flex items-center justify-center">
      ...
      <div id="smriti-book-opened" class="hidden w-full transition-opacity duration-500 opacity-0 z-30">
        ...
        <!-- SLEEK BOOK CONTAINER WITH 2px BORDER MATCHING IMAGE 2 EXACTLY -->
        <div class="relative w-full rounded-[26px] shadow-[0_25px_70px_-15px_rgba(0,0,0,0.28)] border-2 border-[#36132b]/70 bg-[#36132b]">
          ...
          <!-- INNER PAGES SPREAD 1 (PAGES 01 & 02 - MATCHING IMAGE 2 SLEEK PROPORTIONS) -->
          <div id="smriti-spread-1" class="w-full bg-[#faf6ee] rounded-[24px] p-5 md:p-7 relative overflow-hidden text-left h-[500px] md:h-[530px] flex flex-col justify-between" style="background-image: radial-gradient(#d6cebe 1px, transparent 1px); background-size: 20px 20px;">
            ...
            <!-- Center Spine Ribbon Bookmark (Matching Image 2) -->
            <div class="absolute top-0 bottom-0 left-1/2 -translate-x-1/2 w-[2px] bg-amber-900/15 hidden md:block"></div>
            <div class="absolute bottom-0 left-1/2 -translate-x-1/2 w-5 h-7 bg-[#55826b] rounded-t-sm shadow-md hidden md:block"></div>
...
# Replace #about-journal section and its top wave transition in index.html
start_pos = content.find('<!-- PAPER WAVE LAYER TRANSITION')
if start_pos == -1:
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + EXACT_SMRITI_SPEC_HTML + '\n\n' + content[end_pos:]
    print("Successfully updated sleek 2px book border & paper wave transition between Home and Journal!")
