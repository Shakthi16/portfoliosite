...
# 1. Remove all old/duplicate #about-journal sections anywhere in the file
content = re.sub(r'<!-- THE JOURNAL SECTION WITH COVER\.PNG -->.*?</section>\s*(?:<script>.*?</script>)?', '', content, flags=re.DOTALL)
content = re.sub(r'<!-- THE JOURNAL BOOK.*?<!-- /Journal Switcher Script -->', '', content, flags=re.DOTALL)

# 2. Define the Complete Interactive 3-Page Journal Book section
JOURNAL_BOOK_SECTION = """<!-- ABOUT ME: INTERACTIVE 3-PAGE JOURNAL BOOK (DIRECTLY AFTER HOME MAIN PAGE) -->
...
# Insert directly after closing tag of #home (around line 3861)
if '<!-- ABOUT: CINEMATIC EDITORIAL CANVAS -->' in content:
    content = content.replace('<!-- ABOUT: CINEMATIC EDITORIAL CANVAS -->', JOURNAL_BOOK_SECTION + '\n\n<!-- ABOUT: CINEMATIC EDITORIAL CANVAS -->', 1)
    print("Successfully placed #about-journal directly after #home and before #about!")
