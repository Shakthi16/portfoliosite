...
# -------------------------------------------------------------
# PERFECT PHYSICAL JOURNAL BOOK (COVER ENCLOSES PAGES FULLY)
# EXACT SIZING & SMRITI RAWAT MATCHING MOCKUPS 1 & 2
# -------------------------------------------------------------
PERFECT_JOURNAL_HTML = """<!-- ABOUT ME: PHYSICAL JOURNAL BOOK (HARDCOVER FULLY ENCLOSES PAGES) -->
<section class="relative bg-[#f4efe6] text-[#1F1F1F] py-20 overflow-hidden border-b border-amber-900/10 z-20" id="about-journal">
  <div class="max-w-7xl mx-auto px-4 md:px-8 text-center">
    ...
    <div class="relative w-full max-w-[1040px] mx-auto flex items-center justify-center">

      <!-- ==================== CLOSED BOOK COVER STATE ==================== -->
      <div id="perfect-book-cover" class="cursor-pointer transition-all duration-500 z-40 group" onclick="openPerfectBook()">
        <div class="relative">
          <!-- Hardcover Front Image (Exact Proportional Sizing: 440px x 600px) -->
          <div class="w-[330px] md:w-[440px] h-[560px] md:h-[600px] rounded-[32px] shadow-[0_30px_70px_-15px_rgba(0,0,0,0.38)] overflow-hidden relative bg-[#36132b] border-2 border-amber-900/30">
          ...
          <div class="w-7 h-14 bg-[#8b2252] rounded-b-md shadow-md absolute -bottom-6 left-12 pointer-events-none"></div>
        </div>
      </div>

      <!-- ==================== OPENED PHYSICAL HARDCOVER BOOK ==================== -->
      <div id="perfect-book-opened" class="hidden w-full transition-opacity duration-500 opacity-0 z-30">
        
        <!-- HARDCOVER FRAME (BURGUNDY #36132b FULLY ENCLOSING PAGES WITH 20px BORDER) -->
        <div class="relative w-full bg-[#36132b] rounded-[36px] md:rounded-[40px] p-4 md:p-6 shadow-[0_35px_85px_-20px_rgba(0,0,0,0.4)] border border-amber-900/40">
          
          <!-- OUTSIDE RIGHT VERTICAL INDEX TABS (STICKING OUT BEYOND THE HARDCOVER) -->
          <div class="absolute top-12 -right-10 md:-right-12 flex flex-col gap-2 z-50">
            
            <!-- Home / Cover Tab -->
            <button onclick="closePerfectBook()" class="w-10 md:w-12 h-11 bg-[#ebdcc4] hover:bg-[#8b2252] text-gray-800 hover:text-white rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/20 flex items-center justify-center transition-all hover:translate-x-1" title="Close to Cover">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"/></svg>
            </button>

            <!-- Tab 1: Engineering Notes (P. 01-02) -->
            <button id="p-tab-spread1" onclick="switchPerfectSpread('spread1')" class="px-2 md:px-2.5 py-4 bg-[#421835] text-white text-[10px] font-mono font-bold rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1" style="writing-mode: vertical-rl; text-orientation: mixed;">
              Engineering Notes
            </button>

            <!-- Tab 2: Philosophy (P. 03-04) -->
            <button id="p-tab-spread2" onclick="switchPerfectSpread('spread2')" class="px-2 md:px-2.5 py-4 bg-[#9b6b78] hover:bg-[#421835] text-white text-[10px] font-mono font-bold rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1" style="writing-mode: vertical-rl; text-orientation: mixed;">
              Philosophy
            </button>

            <!-- Tab 3: Lessons -->
            <button onclick="switchPerfectSpread('spread2')" class="px-2 md:px-2.5 py-3.5 bg-[#c2a297] text-white text-[10px] font-mono font-bold rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1 hidden sm:flex" style="writing-mode: vertical-rl; text-orientation: mixed;">
              Lessons
            </button>

            <!-- Tab 4: Vision -->
            <button onclick="switchPerfectSpread('spread2')" class="px-2 md:px-2.5 py-3 bg-[#dfcfc7] text-gray-800 text-[10px] font-mono font-bold rounded-r-xl shadow-lg border-t border-r border-b border-amber-900/30 flex items-center justify-center transition-all hover:translate-x-1 hidden sm:flex" style="writing-mode: vertical-rl; text-orientation: mixed;">
              Vision
            </button>

            <!-- Bottom Bookmark Icon -->
            <div class="w-10 md:w-12 h-9 bg-[#55826b] text-white rounded-r-xl shadow-lg flex items-center justify-center">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M5 4a2 2 0 012-2h6a2 2 0 012 2v14l-5-2.5L5 18V4z"/></svg>
            </div>

          </div>

          <!-- INNER PAGES SPREAD 1 (PAGES 01 & 02 - NESTED SAFELY INSIDE HARDCOVER) -->
          <div id="perfect-spread-1" class="w-full bg-[#faf6ee] rounded-[24px] p-6 md:p-8 relative overflow-hidden text-left min-h-[540px]" style="background-image: radial-gradient(#d6cebe 1px, transparent 1px); background-size: 20px 20px;">
            
            <!-- TOP-RIGHT CLOSE BUTTON '✕' -->
            <button onclick="closePerfectBook()" class="absolute top-4 right-5 text-gray-400 hover:text-[#8b2252] text-xl font-bold z-40 p-1 transition-colors" title="Close Journal">
              ✕
            </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 relative z-10 pt-1">
              ...
              <!-- PAGE 01: LEFT SIDE (MATCHING IMAGE 2 MOCKUP EXACTLY) -->
          ...
          <div id="perfect-spread-2" class="hidden w-full bg-[#faf6ee] rounded-[24px] p-6 md:p-8 relative overflow-hidden text-left min-h-[540px]" style="background-image: radial-gradient(#d6cebe 1px, transparent 1px); background-size: 20px 20px;">
...
<script>
  function openPerfectBook() {
    const coverView = document.getElementById('perfect-book-cover');
    const openedView = document.getElementById('perfect-book-opened');
    const spread1 = document.getElementById('perfect-spread-1');
    const spread2 = document.getElementById('perfect-spread-2');
  ...
  function switchPerfectSpread(targetId) {
    const spread1 = document.getElementById('perfect-spread-1');
    const spread2 = document.getElementById('perfect-spread-2');
    const btn1 = document.getElementById('p-tab-spread1');
    const btn2 = document.getElementById('p-tab-spread2');
    ...
    if (targetId === 'spread2') {
      ...
      if (btn1) { btn1.classList.remove('bg-[#9b6b78]'); btn1.classList.add('bg-[#421835]'); }
    }
...
if start_pos != -1 and end_pos != -1:
    content = content[:start_pos] + PERFECT_JOURNAL_HTML + '\n\n' + content[end_pos:]
    print("Successfully fixed book cover enclosing pages with 20px hardcover margin!")
