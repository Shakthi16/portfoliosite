...
if start_pos != -1:
    ...
    # Nudge button further left and slightly up to align centered on the dotted placeholder line
    new_button = '''<!-- WATTPAD INTERACTIVE LINK BUTTON EMBEDDED PERFECTLY ON DOTTED PLACEHOLDER LINE -->
            <a href="https://www.wattpad.com/story/411657344-the-threadweaver's-knot" target="_blank" rel="noopener noreferrer" 
               class="absolute bottom-[28px] md:bottom-[36px] right-[130px] md:right-[185px] z-40 bg-[#4A1224] hover:bg-[#6B2137] text-white font-bold text-[9.5px] sm:text-[10.5px] px-2.5 sm:px-3.5 py-0.5 sm:py-1 rounded-full shadow-md transition-all hover:scale-105 flex items-center gap-1 border border-amber-200/40 group">
    ...
    updated_spread3 = re.sub(r'<!-- WATTPAD INTERACTIVE LINK BUTTON EMBEDDED PERFECTLY ON DOTTED PLACEHOLDER LINE -->[\s\S]*?</a>', new_button, old_spread3)

    new_html = html[:start_pos] + updated_spread3 + html[end_pos + len('</div>'):]

    with open('e:/portfoliosite/index.html', 'w', encoding='utf-8') as f:
        f.write(new_html)
    print("SUCCESSFULLY NUDGED WATTPAD BUTTON FURTHER LEFT AND UP!")
else:
